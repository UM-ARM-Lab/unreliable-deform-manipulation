...
class LinearTFModel(base_model.BaseModel):
    ...
    def __init__(self, args, batch_size, N, M, L, P, dt, n_steps, seed=0):
        ...
        self.k = tf.placeholder(tf.float32, shape=(batch_size, self.n_steps + 1), name="k")

        a_init = np.random.randn(N, M).astype(np.float32) * 1e-6
        a_init[0, 0] = 1
        a_init[1, 1] = 1
        k_init = np.random.randn(N, P).astype(np.float32) * 1e-6
        k_init[0, 0] = 1
        k_init[1, 1] = 1
        ...
        self.K = tf.get_variable("K", initializer=k_init)
