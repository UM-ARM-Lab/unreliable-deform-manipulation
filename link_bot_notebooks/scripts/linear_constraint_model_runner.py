...
def train(args):
    ...
    x = log_data[:, :, :]
    dt = x[0, 1, 0] - x[0, 0, 0]
    model = m.LinearConstraintModel(vars(args), x.shape[0], args.N, args.M, args.L, args.P, args.Q, dt, x.shape[1] - 1,
                                    seed=args.seed)
...
def model_only(args):
    model = m.LinearConstraintModel(vars(args), 250, args.N, args.M, args.L, args.P, args.Q, 0.1, 50)
    if args.log:
        model.init()
        log_path = experiments_util.experiment_name(args.log)
...
def evaluate(args):
    ...
    x = log_data[:, :, :]
    dt = x[0, 1, 0] - x[0, 0, 0]
    model = m.LinearConstraintModel(vars(args), x.shape[0], args.N, args.M, args.L, args.P, dt, x.shape[1] - 1)
    model.load()
...
def main():
    ...
    parser.add_argument("-P", help="dimensions in control input", type=int, default=2)
    parser.add_argument("-Q", help="dimensions in control input", type=int, default=1)
