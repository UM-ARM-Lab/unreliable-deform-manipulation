...
class ClassifierDataset:
    ...
    def parser(self, local_env_shape, n_state, n_action):
        def _parser(serialized_example):
            features = {
                ...
                'local_env_rows': tf.FixedLenFeature([1], tf.float32),
                'local_env_cols': tf.FixedLenFeature([1], tf.float32),
    ...
    def make_features_dict(cls,
        ...
        features = {
            ...
            'local_env_rows': float_feature(np.array([local_env_rows])),
            'local_env_cols': float_feature(np.array([local_env_cols])),
