...
def main():
    ...
    root_output_directory = args.indir.parent / (args.indir.name + "-state-only")
    ...
    for mode in ['train', 'test', 'val']:
        ...
        dataset = classifier_dataset.cf_get_dataset(mode=mode, num_epochs=1, batch_size=0, shuffle=False)
    ...
    if not args.no_plot:
        ...
        plt.hist(pre_dists, label='pre', alpha=0.5, bins=100)
