...
def generate(args):
    data = np.ndarray((args.num_trajectories, args.trajectory_length, 8))
    ...
    for t in range(args.num_trajectories):
        ...
        for s in range(args.trajectory_length):
            ...
            time = s * args.dt
            data[t][s] = [time, 0, o[0], o[1], o[0] + o[1], 2 * o[0] - 4 * o[1], vx, vy]

    np.save(args.data_outfile, data)

    sdf = np.zeros([200, 200]).astype(np.float32)
    sdf_gradient = np.zeros([200, 200, 2]).astype(np.float32)
    sdf_resolution = np.array([0.01, 0.01]).astype(np.float32)
    np.savez(args.sdf_outfile, sdf=sdf, sdf_gradient=sdf_gradient, sdf_resolution=sdf_resolution)
...
def main():
    ...
    generate_parser.add_argument("data_outfile", help='output file name')
    generate_parser.add_argument("sdf_outfile", help='output file name')
