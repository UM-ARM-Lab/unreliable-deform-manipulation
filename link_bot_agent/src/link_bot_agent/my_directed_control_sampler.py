...
class MyDirectedControlSampler(oc.DirectedControlSampler):
    ...
    @classmethod
    def plot_dual_sdf(cls, sdf, start, goal, d_path, k_path):
        ...
        plt.plot(d_path[:, 0], d_path[:, 1], label='d path', linewidth=3)
        plt.plot(k_path[:, 0], k_path[:, 1], label='k path', linewidth=3)
        plt.xlabel("o0")
        plt.ylabel("o1")
        plt.axis("equal")
        plt.legend()
