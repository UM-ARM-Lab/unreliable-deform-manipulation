...
double Manipulator::transitionCost(const std::vector<double>& q1, const double t1, const std::vector<double>& q2,
...
Matrix6Xd Manipulator::getJacobianServoFrame(const robot_model::RobotState& state, const robot_model::LinkModel* link,
...
std::pair<Eigen::VectorXd, double> Manipulator::servoNullSpace(const robot_state::RobotState& startingState,
															   ...
															   const double displacement) const
{
  ...
  for (int itr = 0; itr < maxItr; ++itr)
  {
	...
	const Eigen::Matrix<double, 6, 1> error =
		(Eigen::Matrix<double, 6, 1>() << poseError.translation(), twistError.tail<3>()).finished();
	...
	// and then servo in the null space of the jacobian some distance
	{
	  ...
	  const Eigen::VectorXd nullspaceDirection =
		  sgn(displacement) * (nullspaceProjector * Eigen::VectorXd::Ones(ndof)).normalized();

	  const Eigen::VectorXd correctionStep =
		  EigenHelpers::UnderdeterminedSolver(jacobian, error, dampingThreshold, damping);
...
bool Manipulator::cartesianPath(const PoseSequence& worldGoalPoses, const Pose& robotTworld,
								const robot_state::RobotState& currentState,
								planning_scene::PlanningSceneConstPtr scene,
								trajectory_msgs::JointTrajectory& cmd) const
{
  ...
  const auto stateCostFn = [&](const std::vector<double>& q) -> double { return stateCost(q); };
  const auto transitionCostFn = [&](const std::vector<double>& q1, const double t1, const std::vector<double>& q2,
									const double t2) -> double { return transitionCost(q1, t1, q2, t2); };
  ...
  std::vector<int> const cheapestPath =
	  mps::viterbi(trellis, times, stateCostFn, transitionCostFn, maxAllowedTransitionCost);
...
bool Manipulator::nullspacePath(const robot_state::RobotState& startingState,
								const Eigen::Vector3d& flangeReferencePoint,
								planning_scene::PlanningSceneConstPtr scene, const double displacement,
